...
export default function BackgroundRemover() {
  ...
  const addFiles = (e) => {
    const imgs = Array.from(e.target.files).filter(f => /\.(jpg|jpeg|png)$/i.test(f.name));
  ...
  const remove = useCallback(async () => {
    ...
    for (const f of files) {
      try {
        const { ok, result, error } = await window.pdfUnlocker.removeBackground({ filePath: f.path });
        if (ok) {
          out.push({ name: f.name, ...result, success: true });
        } else {
          out.push({ name: f.name, success: false, error });
        }
      } catch (err) {
  ...
  return (
    <ToolLayout title="Background Remover" description="Automatically remove background from images using AI. Outputs PNG with transparency.">
      <InputPanel label="Upload Images">
        <div onClick={() => fileRef.current?.click()} className="drop-zone p-6 text-center cursor-pointer mb-3">
          <p className="text-sm text-gray-500 dark:text-gray-400">Click to select images (JPG or PNG)</p>
        </div>
        <input ref={fileRef} type="file" accept=".jpg,.jpeg,.png" multiple className="hidden" onChange={addFiles} />
        {files.length > 0 && (
        ...
        <ActionButton onClick={remove} disabled={processing || !files.length}>
          {processing ? 'Processing...' : 'Remove Background'}
        </ActionButton>
        <p className="text-xs text-gray-400 mt-2">Uses AI (ONNX) — first run downloads model (~50MB), subsequent runs are offline.</p>
      </InputPanel>
          ...
          <div className="space-y-1.5">
            {results.map((r, i) => (
              <div key={i} className="flex items-center gap-2 text-sm">
                ...
                {r.success && <span className="text-xs text-gray-400">→ PNG</span>}
